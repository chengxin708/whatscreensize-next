...
import type { MonitorProduct, MonitorProductsBySpec } from '@/types/product';

export default function MonitorProductsPage() {
  const router = useRouter();
  const [products, setProducts] = useState<MonitorProductsBySpec>({});
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const keys = Object.keys(data.products).sort((a, b) => {
        // Sort by screen size first, then resolution
        const [sizeA] = a.split('-');
        const [sizeB] = b.split('-');
        return Number(sizeA) - Number(sizeB) || a.localeCompare(b);
      });
      setSpecKeys(keys);
  ...
  const currentProducts: MonitorProduct[] = products[activeTab] || [];

  async function handleDelete(id: number) {
    if (!window.confirm('Are you sure you want to delete this product?')) return;

    try {
      const res = await fetch(`/api/monitor-products/${id}`, { method: 'DELETE' });
  ...
  async function handleMoveUp(index: number) {
    ...
    try {
      const res = await fetch('/api/monitor-products/reorder', {
        method: 'PUT',
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text-dim)' }}>
            Manage monitor product listings by specification
          </p>
        </div>
        <button
          onClick={() => router.push('/admin/monitor-products/new')}
      ...
      <div className="flex gap-4">
        {/* Vertical spec tabs */}
        <div
          className="flex flex-col gap-1 shrink-0 w-[160px] rounded-xl p-2 self-start overflow-y-auto"
          style={{
        ...
        >
          {specKeys.map((spec) => {
            const count = (products[spec] || []).length;
            return (
              <button
                key={spec}
                onClick={() => setActiveTab(spec)}
                className="min-h-[36px] px-3 rounded-lg text-sm font-medium transition-all duration-200 flex items-center justify-between gap-1 text-left cursor-pointer"
                style={{
                  background:
                    activeTab === spec
                      ...
                      : 'transparent',
                  color:
                    activeTab === spec ? '#818cf8' : 'var(--text-muted)',
                  border:
                    activeTab === spec
              ...
              >
                <span className="truncate text-xs">{formatSpecLabel(spec)}</span>
                <span
                  ...
                  style={{
                    background:
                      activeTab === spec
                        ...
                        : 'var(--input-bg)',
                    color:
                      activeTab === spec ? '#a5b4fc' : 'var(--text-dim)',
                  }}
                >
                  {count}
                </span>
          ...
          {specKeys.length === 0 && (
            <p className="text-xs py-2 px-2" style={{ color: 'var(--text-dim)' }}>
          ...
          {currentProducts.length === 0 ? (
            <div className="px-6 py-12 text-center">
              <p className="text-sm" style={{ color: 'var(--text-dim)' }}>
                {activeTab
                  ? `No products for ${formatSpecLabel(activeTab)} yet.`
                  : 'Select a spec category to view products.'}
              </p>
            </div>
          ) : (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <ActionBtn
                            label="Edit"
                            onClick={() =>
                              router.push(`/admin/monitor-products/${product.id}`)
                            }
                          >
